...
// Exact CSS from Striker Player Card.html
...
export function Standard1StrikerCard({ batter, player, team }: Props) {
  const headshot = player?.headshotCloudinaryId

  const topLabels = ['Runs', 'Balls', 'Fours', 'Sixes', 'S/R']
  const bottomValues = [
    batter.runs,
    batter.balls,
    batter.fours,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: 55, fontWeight: 1000 as number, width: 110, height: 80, lineHeight: '80px', textAlign: 'right' }}>
              {batter.runs}
            </span>
            <span style={{ fontSize: 40, fontWeight: 1000 as number, width: 60, height: 80, lineHeight: '90px', marginLeft: 20 }}>
              ({batter.balls})
            </span>
          </div>
        </div>
