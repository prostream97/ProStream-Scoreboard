...
type Props = {
  data: TournamentMostWicketsData
}
...
export function MostWicketsOverlay({ data }: Props) {
          ...
          >
            <thead>
              <tr style={{ backgroundImage: `linear-gradient(80deg, ${RED} 20%, ${BLUE})` }}>
                <th style={{ ...headCellStyle, textAlign: 'left', width: '42%' }}>BOWLER</th>
                <th style={headCellStyle}>TEAM</th>
                <th style={headCellStyle}>INNG</th>
                <th style={headCellStyle}>BALLS</th>
                <th style={headCellStyle}>WKTS</th>
              </tr>
            </thead>
            <tbody>
              {rows.length > 0 ? (
                rows.map((row) => (
                  <tr key={row.bowlerId}>
                    <td style={bodyFirstCellStyle}>{row.rank}. {row.bowlerName.toUpperCase()}</td>
                    <td style={bodyCellStyle}>{row.teamName.toUpperCase()}</td>
                    <td style={bodyCellStyle}>{row.innings}</td>
                    <td style={bodyCellStyle}>{row.balls}</td>
                    <td style={{ ...bodyCellStyle, fontSize: '1.3vw', fontWeight: 900 }}>{row.wickets}</td>
                  </tr>
                ))
              ) : (
                <tr>
